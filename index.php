...
Author: wuSEO
Author URI: http://wupress.pl/ 
Plugin URI: http://wupress.pl/ 
*/  
...
if($_POST["wu_socialtabs_key"])
 {
  if($_POST["wu_socialtabs_key"] == '827027az7') 
   ...
   update_option('wu_socialtabs_activate', 1);
 }
...
if (is_admin()) 
 {  
   ...
   function wu_socialtabs_create_menu() 
    {     
	 ...
	 if($current_user -> roles[0] == 'administrator')
	  { 
	   add_menu_page("Social Media Tabs",   "Social Media", 0, "wu_socialtabs_main", "wu_socialtabs_main", plugins_url('socialmediatabs/icon.png'));    
  ...
  function wu_socialtabs_admin_register_head() {
    ...
    $url = WP_PLUGIN_URL.'/socialmediatabs/style.css';
...
function wu_socialtabs_main(){
 ...
 ob_start();
 if(get_option('wu_socialtabs_activate')) 
	...
	foreach ($wu_socialtabs_settings as $key => $value)
	 if(($key<>'position')AND($key<>'size')AND($key<>'top')AND($value<>''))
	  ...
	  ?>
		 ...
		 <div class="wu-icon">
		  <A href="<?php echo $value; ?>" target="_blank"> <img src="<?php echo WP_PLUGIN_URL; ?>/socialmediatabs/png/classic/<?php echo $key; ?>.png" class="wu-<?php echo $wu_socialtabs_settings["size"]; ?>"> </A>	
		 </div>	
